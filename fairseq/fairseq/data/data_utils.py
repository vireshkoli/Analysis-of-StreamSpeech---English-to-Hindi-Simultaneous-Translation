...
#         """
#         Robust copy used by collate_tokens. If src is empty (numel()==0) we
#         print a short debug trace and skip the copy to avoid IndexError.
#         This preserves existing behavior for non-empty src.
#         """
#         # defensive: report empty src rather than crash
#         if src.numel() == 0:
#             import sys, traceback
#             print("\n[DEBUG copy_tensor] detected EMPTY src tensor (numel==0)", file=sys.stderr)
#             print("[DEBUG] src.shape:", tuple(src.shape), " dst.shape:", tuple(dst.shape), file=sys.stderr)
#             # short traceback so we can see caller chain (limit 6 frames)
#             traceback.print_stack(limit=6, file=sys.stderr)
#             # leave dst as-is (should be already padded/initialized by caller)
#             return

#         # if dst is smaller than src, copy as much as fits (defensive)
#         if dst.numel() < src.numel():
#             # try to copy prefix that fits and warn
#             try:
#                 dst[:src.numel()].copy_(src)
#             except Exception:
#                 # fallback: attempt a safe elementwise copy
#                 for i in range(min(dst.numel(), src.numel())):
#                     dst[i] = src[i]
#             return

#         # original logic: handle move_eos_to_beginning if set in caller's scope
#         try:
...
#                 else:
#                     dst[0] = eos_idx
#                 # copy remaining tokens if any
#                 if src.numel() > 1:
#                     dst[1:1 + (src.numel() - 1)].copy_(src[:-1])
#             else:
#                 dst.copy_(src)
#         except NameError:
#             # if move_eos_to_beginning or eos_idx not defined in this scope,
#             # fall back to a simple copy (preserves previous behaviour in
#             # environments where those are not defined as globals).
#             dst.copy_(src)
