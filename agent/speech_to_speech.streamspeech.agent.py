...
#         def beam_hypo_to_str(hypo, source_unigram):
#             # Convert one beam-hypo (list of beam dicts) -> python string
#             i_beam = 0
#             tmp_tensor = hypo[i_beam].get("tokens", torch.tensor([], dtype=torch.long))
#             if isinstance(tmp_tensor, torch.Tensor):
#                 tmp = tmp_tensor.int()
#             else:
#                 tmp = torch.tensor(tmp_tensor, dtype=torch.long)

#             if tmp.numel() == 0 and "org_tokens" in hypo[i_beam]:
#                 tmp = hypo[i_beam]["org_tokens"].int()

#             # make python list of ints
#             try:
#                 token_ids = tmp.cpu().tolist()
#             except Exception:
#                 token_ids = tmp.tolist()

#             # decode -> string
#             text = "".join([source_unigram[int(c)] for c in token_ids])
#             text = text.replace("_", " ").replace("▁", " ")
#             text = text.replace("<unk>", " ").replace("<s>", "").replace("</s>", "")
#             text = text.lstrip()
#             # ensure it's a Python str
#             if not isinstance(text, str):
#                 text = str(text)
#             return text

#         def beam_hypo_to_ids(hypo):
#             # Convert one beam-hypo -> python list of ints (with same fallback logic)
#             i_beam = 0
#             tmp_tensor = hypo[i_beam].get("tokens", torch.tensor([], dtype=torch.long))
#             if isinstance(tmp_tensor, torch.Tensor):
#                 tmp = tmp_tensor.int()
...
#         )
#         print("DEBUG_ASR_finalized:", finalized_asr)
#         if len(finalized_asr) and len(finalized_asr[0]) and isinstance(finalized_asr[0][0], dict) and "lprobs" in finalized_asr[0][0]:
#             try:
#                 argmax_ids = torch.argmax(finalized_asr[0][0]["lprobs"][0], dim=-1).tolist()
#             except Exception as _e:
#                 argmax_ids = str(_e)
#             print("DEBUG_ASR_argmax_ids:", argmax_ids)
#             try:
#                 print("DEBUG_ASR_source_unigram_idx0:", repr(self.dict["source_unigram"][0]))
#             except Exception as _e:
#                 print("DEBUG_ASR_source_unigram_idx0_error:", _e)
#         else:
#             argmax_ids = []

#         uniq_argmax = sorted(set(argmax_ids)) if isinstance(argmax_ids, (list, tuple)) else []
#         print("DEBUG_ASR_unique_argmax_ids:", uniq_argmax)
#         for _id in uniq_argmax[:40]:
#             try:
#                 print("DEBUG_ASR_map:", _id, repr(self.dict["source_unigram"][_id]))
#             except Exception as _e:
#                 print("DEBUG_ASR_map_error:", _id, _e)


#         # lprobs checks
#         if len(finalized_asr) and len(finalized_asr[0]) and isinstance(finalized_asr[0][0], dict) and "lprobs" in finalized_asr[0][0]:
#             asr_probs = torch.exp(finalized_asr[0][0]["lprobs"])
#             lp = finalized_asr[0][0]["lprobs"]
#             print("DEBUG_LPROBS_SHAPE:", lp.shape)   # expect (1, T, V) or (T, V)
#             print("DEBUG_DICT_SIZE:", len(self.dict["source_unigram"]))
#             print("DEBUG_LPROBS_VSIZE:", lp.shape[-1])
#             assert lp.shape[-1] == len(self.dict["source_unigram"]), "vocab size mismatch!"
#         else:
#             asr_probs = None
#             lp = None
#             print("DEBUG_LPROBS_SHAPE: no lprobs in finalized_asr")

#         # Build hyps (strings) and also capture src_ctc_indices from first hypo safely
#         hyps = []
#         for i, hypo in enumerate(finalized_asr):
#             text = beam_hypo_to_str(hypo, self.dict["source_unigram"])
#             hyps.append(text)

#             # preserve existing behavior: write to file and print
...
#         # --- Ensure src_ctc_indices/src_ctc_index exist for later code ---
#         if len(finalized_asr) and len(finalized_asr[0]):
#             first_ids = beam_hypo_to_ids(finalized_asr[0])
#             if isinstance(first_ids, (list, tuple)):
#                 try:
#                     src_ctc_indices = torch.tensor(first_ids, device=self.device, dtype=torch.long)
#                 except Exception:
#                     src_ctc_indices = torch.tensor(first_ids, dtype=torch.long)
#             else:
#                 # fallback empty
#                 src_ctc_indices = torch.tensor([], device=self.device, dtype=torch.long)
#             first_hypo = finalized_asr[0][0]
#             src_ctc_index = first_hypo.get("index", None)
#         else:
#             src_ctc_indices = torch.tensor([], device=self.device, dtype=torch.long)
#             src_ctc_index = None
#         # ---------------------------------------------------------------------
...
#         )
#         # guard lprobs for ST as well
#         if len(finalized_st) and len(finalized_st[0]) and isinstance(finalized_st[0][0], dict) and "lprobs" in finalized_st[0][0]:
#             st_probs = torch.exp(finalized_st[0][0]["lprobs"])
#         else:
#             st_probs = None

#         # Extract tgt_ctc_indices robustly (first beam fallback)
#         tgt_ctc_indices = torch.tensor([], device=self.device, dtype=torch.long)
#         tgt_ctc_index = None
#         for i, hypo in enumerate(finalized_st):
#             i_beam = 0
#             tmp_tensor = hypo[i_beam].get("tokens", torch.tensor([], dtype=torch.long))
#             if isinstance(tmp_tensor, torch.Tensor):
#                 tmp_tmp = tmp_tensor.int()
#             else:
#                 tmp_tmp = torch.tensor(tmp_tensor, dtype=torch.long)

#             if tmp_tmp.numel() == 0 and "org_tokens" in hypo[i_beam]:
#                 tmp_tmp = hypo[i_beam]["org_tokens"].int()

#             # make python list
#             try:
#                 token_ids = tmp_tmp.cpu().tolist()
#             except Exception:
#                 token_ids = tmp_tmp.tolist()

#             # set tgt variables (last iteration will leave the values from the last hypo; you can break if you only want first)
#             try:
#                 tgt_ctc_indices = torch.tensor(token_ids, device=self.device, dtype=torch.long)
#             except Exception:
#                 tgt_ctc_indices = torch.tensor(token_ids, dtype=torch.long)
#             tgt_ctc_index = hypo[i_beam].get("index", None)

#             # build text if you want (kept original cleanup behavior)
#             text = "".join([self.dict["ctc_target_unigram"][c] for c in token_ids]) if len(token_ids) else ""
#             text = text.replace("_", " ")
#             text = text.replace("▁", " ")
...
#         # now downstream logic expecting src_ctc_indices / tgt_ctc_indices
