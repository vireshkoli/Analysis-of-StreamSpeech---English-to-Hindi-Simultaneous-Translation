...
class SpeechToUnit2passCTCASRSTMultitaskTaskCriterion(
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        # --- DEBUG INSTRUMENTATION FOR CTC SHAPE MISMATCH ---
        try:
            print("DEBUG CTC: lprobs.shape=", getattr(lprobs, "shape", None))
            print("DEBUG CTC: target.shape=", getattr(target, "shape", None))
            try:
                vals = input_lengths.tolist() if hasattr(input_lengths, "tolist") else None
            except Exception:
                vals = None
            print("DEBUG CTC: input_lengths.shape=", getattr(input_lengths, "shape", None), "vals=", vals)
            try:
                tvals = target_lengths.tolist() if hasattr(target_lengths, "tolist") else None
            except Exception:
                tvals = None
            print("DEBUG CTC: target_lengths.shape=", getattr(target_lengths, "shape", None), "vals=", tvals)
        except Exception as e:
            print("DEBUG CTC: failed to print shapes:", e)
